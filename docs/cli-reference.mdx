...
The `delve` CLI provides commands for taxonomy generation and API server management:

```bash
# Generate taxonomy from data
delve run DATA_SOURCE [OPTIONS]

# Start API server for JavaScript/TypeScript integration
delve serve [OPTIONS]
```
...
<ParamField path="--fast-llm" type="string" default="anthropic/claude-haiku-4-5-20251001">
...
Start the Delve REST API server for integration with JavaScript/TypeScript frontends.
...
<ParamField path="--host" type="string" default="127.0.0.1">
  ...
  delve serve --host 0.0.0.0

  # Localhost only (default)
  delve serve --host 127.0.0.1
  ```
</ParamField>
...
<ParamField path="--port" type="integer" default="8000">
  ...
  ```bash
  delve serve --port 3000
  ```
</ParamField>

<ParamField path="--reload" type="flag">
  Enable auto-reload for development. The server restarts when source files change.
  ...
  <Warning>
    Only use `--reload` in development. It's not suitable for production.
  </Warning>
</ParamField>

<ParamField path="--cors-origins" type="string" default="*">
  ...
  delve serve --cors-origins "*"
  ```

  <Tip>
    In production, always specify explicit origins instead of using `*`.
  </Tip>
</ParamField>
...
When the server starts, you get:

| Endpoint | Description |
|----------|-------------|
| `http://localhost:8000/docs` | Interactive Swagger UI documentation |
| `http://localhost:8000/openapi.json` | OpenAPI schema (for code generation) |
| `http://localhost:8000/health` | Health check endpoint |
| `http://localhost:8000/taxonomies` | Taxonomy generation API |

<Info>
  See the [REST API Reference](/api-reference) for complete endpoint documentation.
</Info>
