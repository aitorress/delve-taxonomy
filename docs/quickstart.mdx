...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    const delve = new Delve({
      maxNumClusters: 5,
      useCase: 'Categorize user feedback',
    });
    ...
    <Check>
      No server required! The TypeScript SDK runs directly in Node.js.
    </Check>
    ...
    // Access metadata
    console.log(`Processed ${result.metadata.numDocuments} documents`);
    console.log(`Duration: ${result.metadata.runDurationSeconds}s`);
    ```

    ## Next Steps

    - Explore [configuration options](/typescript-sdk#configuration)
    - Try [predefined taxonomies](/typescript-sdk#with-predefined-taxonomy)
    - See more [examples](/examples)
  </Tab>
</Tabs>
