...
<Tabs>
  ...
  <Tab title="REST API">
    ...
    const result = await fetch(`http://localhost:8000/taxonomies/${job.job_id}`);
    const data = await result.json();

    if (data.status === 'completed') {
      console.log('Categories:', data.result.taxonomy);
    }
    ```

    <Check>
      The API supports real-time streaming via Server-Sent Events (SSE) for progress updates.
    </Check>

    ## Next Steps

    - Read the full [REST API Reference](/api-reference)
    - See [TypeScript Integration](/typescript-integration) for type-safe clients
